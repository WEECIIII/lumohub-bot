...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addUserOption(option => 
            option.setName('user')
                .setDescription('Check keys of a specific user (Admin only)')
                .setRequired(false))
        .addStringOption(option => 
            option.setName('key')
                .setDescription('Check status of a specific key string (Admin only)')
                .setRequired(false))
        .toJSON(),
    new SlashCommandBuilder()
        .setName('revoke')
...
client.on('interactionCreate', async interaction => {
    ...
    if (commandName === 'keyinfo') {
        ...
        // Admin checking a specific key string
        if (specificKey) {
            if (!hasKeysRole(member)) {
                return interaction.reply({ content: '❌ You do not have permission to check specific keys!', ephemeral: true });
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .setFooter({ text: 'LumoHub • Admin View' });
            return interaction.reply({ embeds: [embed], ephemeral: true });
        }

        // Checking user keys
        const userToCheck = targetUser || user;
        if (targetUser && !hasKeysRole(member)) {
            return interaction.reply({ content: '❌ You do not have permission to check other users!', ephemeral: true });
        }

        // Find keys generated by this user
        const myKeys = [];
        for (const [key, data] of validKeys.entries()) {
            if (data.generatedBy === userToCheck.id) {
                myKeys.push({ key, expiresAt: data.expiresAt, duration: data.duration });
            }
        }

        if (myKeys.length === 0) {
            const msg = targetUser ? `❌ <@${targetUser.id}> has no active keys.` : '❌ You have no active keys. Use `/generate` to get one!';
            return interaction.reply({ content: msg, ephemeral: true });
        }

        const embedTitle = targetUser ? `⏳ Active Keys for ${targetUser.username}` : '⏳ Your Active Keys';
        const embed = new EmbedBuilder()
            .setColor(0x7c3aed)
            .setTitle(embedTitle)
