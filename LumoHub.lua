...
PlayerTab:CreateToggle({
    Name = "God Mode (Auto-Heal) [SAFE]",
    CurrentValue = false,
    Flag = "Player_GodModeSafe",
    Callback = function(Value)
        ...
        if Value then
            ...
            Rayfield:Notify({Title = "Safe God Mode", Content = "Auto-healing enabled. Safe from anti-cheat.", Duration = 4})
        else
...
local DesyncLoop
PlayerTab:CreateToggle({
    Name = "Invincibility Desync [RISKY - MIGHT KICK]",
    CurrentValue = false,
    Flag = "Player_Desync",
    Callback = function(Value)
        if Value then
            ...
            Rayfield:Notify({Title = "Desync Enabled", Content = "Warning: This may trigger anti-cheat kicks!", Duration = 4})
        else
...
local HitboxLoop
PlayerTab:CreateToggle({
    Name = "Ghost Hitboxes [RISKY - MIGHT KICK]",
    CurrentValue = false,
    ...
    Callback = function(Value)
        if Value then
            ...
            Rayfield:Notify({Title = "Hitboxes Ghosted", Content = "Warning: Modifying hitboxes may trigger anti-cheat kicks!", Duration = 4})
        else
...
PlayerTab:CreateInput({
    ...
    Callback = function(Text)
        ...
        pcall(function()
            ...
            if foundLevel then
                Rayfield:Notify({Title = "Level Spoofed!", Content = "Your level is now " .. tostring(num) .. ".", Duration = 4})
            else
